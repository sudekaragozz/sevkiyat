...
type CargoForm = { employee: number | ''; origin_branch: number | ''; destination_branch: number | ''; sefer: number | ''; recipient_name: string; recipient_phone: string; desi: string; payment_type: string }

const initialForm: CargoForm = { employee: '', origin_branch: '', destination_branch: '', sefer: '', recipient_name: '', recipient_phone: '', desi: '', payment_type: 'Peşin' }
...
export default function App() {
  ...
  const loadReferences = useCallback(async () => {
    ...
    try {
      const responses = await Promise.all([fetch('/api/employees/'), fetch('/api/branches/'), fetch('/api/sefer/'), fetch('/api/vehicles/')])
      if (responses.some((response) => !response.ok)) throw new Error('Çalışan, şube, sefer veya araç listesi alınamadı.')
      const [employeeData, branchData, seferData, vehicleData] = await Promise.all(responses.map((response) => response.json()))
      setEmployees(normalize(employeeData)); setBranches(normalize(branchData)); setSefers(normalize(seferData)); setVehicles(normalize(vehicleData))
  ...
  const updateReference = (field: 'employee' | 'origin_branch' | 'destination_branch' | 'sefer', value: string) => setForm((current) => ({ ...current, [field]: Number(value) }))
  ...
  const submitCargo = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (form.employee === '' || form.origin_branch === '' || form.destination_branch === '' || form.sefer === '') { setSubmitError('Çalışan, çıkış şubesi, varış şubesi ve sefer seçilmelidir.'); setSubmitting(false); return }
    const sefer = Number(form.sefer); const origin = Number(form.origin_branch); const destination = Number(form.destination_branch)
    const payload = { ...form, employee: Number(form.employee), origin_branch: origin, destination_branch: destination, sefer, history: [{ sefer, origin_branch: origin, destination_branch: destination, sequence_no: 1, status: 'CREATED' }] }
